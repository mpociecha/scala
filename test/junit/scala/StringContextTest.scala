...
object StringContextTestUtils {
  ...
  implicit class StringContextOps(val sc: StringContext) extends AnyVal {
    ...
    private def applyProperLocale(numberAsString: String): String = numberAsString match {
      case numberPattern(intPart, fractionalPart, suffix) =>
        val decimalPlaces = fractionalPart.length
        val number = s"$intPart.$fractionalPart".toDouble
        ("%." + decimalPlaces + "f").format(number) + suffix
    }
  }
}
